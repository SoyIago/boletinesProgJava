...
public class ejercicio8a {
    ...
    public static void main(String[] args) {
        ...
        boolean valid_intOfInput = false, valid_CharOfInput = false;
        int intOfInput = 0;
        char charOfInput = 'a';
        ...
        do {
            ...
            if (stgIntOfInput.matches("\\d{1,}")) {
                intOfInput = Integer.parseInt(stgIntOfInput);
                if (intOfInput <= stgInput.length()) valid_intOfInput = true;
            }
        } while (!valid_intOfInput);
        
        do {
            System.out.print("Introduce un caracter: ");
            stgCharOfInput = keyboard.nextLine();
            if (stgCharOfInput.length() == 1) {
                charOfInput = stgCharOfInput.charAt(0);
                valid_CharOfInput =  true;
            }
        } while (!valid_CharOfInput);
